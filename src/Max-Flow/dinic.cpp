...
Graph generate_reverse_edges(const Graph &G) {
  ...
  parlay::parallel_for(0, G_rev.n, [&](NodeId u) {
    parlay::parallel_for(G_rev.offsets[u], G_rev.offsets[u + 1], [&](EdgeId i) {
      EdgeId id = std::get<2>(edgelist[i]);
      if (id & 1) {
        std::get<1>(rev_id[id / 2]) = i - G_rev.offsets[u];
      } else {
        std::get<0>(rev_id[id / 2]) = i - G_rev.offsets[u];
      }
    });
  });
  ...
  parlay::parallel_for(0, G_rev.n, [&](NodeId u) {
    parlay::parallel_for(G_rev.offsets[u], G_rev.offsets[u + 1], [&](EdgeId i) {
      ...
      uint32_t rev = G_rev.edges[i].rev;
      assert(G_rev.edges[G_rev.offsets[v] + rev].v == u);
      if (w) {
        assert(G_rev.edges[G_rev.offsets[v] + rev].w == 0);
      } else {
        assert(G_rev.edges[G_rev.offsets[v] + rev].w != 0);
      }
    });
...
void run(Algo &algo, [[maybe_unused]] const Graph &G, NodeId s, NodeId t) {
  printf("source %u, target %u\n", s, t);
  double total_time = 0;
  FlowTy max_flow;
